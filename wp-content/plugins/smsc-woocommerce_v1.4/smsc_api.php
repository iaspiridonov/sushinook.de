<?php
// RU.SMSC.KZ API (ru.smsc.kz) версия 3.7 (09.10.2017)
...
define("SMTP_FROM", "api@ru.smsc.kz");     // e-mail адрес отправителя
...
{
	return mail("send@send.ru.smsc.kz", "", SMSC_LOGIN.":".SMSC_PASSWORD.":$id:$time:$translit,$format,$sender:$phones:$message", "From: ".SMTP_FROM."\nContent-Type: text/plain; charset=".SMSC_CHARSET."\n");
}
...
{
	$url = $_url = (SMSC_HTTPS ? "https" : "http")."://ru.smsc.kz/sys/$cmd.php?login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;

	$i = 0;
	do {
		if ($i++)
			$url = str_replace('://ru.smsc.kz/', '://www'.$i.'.ru.smsc.kz/', $_url);
...
// list($sms_id, $sms_cnt, $cost, $balance) = send_sms("79999999999", "http://ru.smsc.kz\nRU.SMSC.KZ", 0, 0, 0, 0, false, "maxsms=3");
