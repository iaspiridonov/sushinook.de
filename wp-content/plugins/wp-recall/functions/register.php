...
function rcl_secondary_password($fields){
    ...
    $fields .= '<input placeholder="'.__('Repeat the password','wp-recall').'" required id="secondary-pass-user" type="password" value="'.$_REQUEST['user_secondary_pass'].'" name="user_secondary_pass">
...
function rcl_custom_fields_regform($field){
    ...
    if($get_fields){
        ...
        foreach((array)$get_fields as $custom_field){
            ...
            $field .= $cf->get_input($custom_field,$_POST[$custom_field['slug']]).$star;
